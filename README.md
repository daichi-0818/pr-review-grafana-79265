...
## 1.1 忙しい方向けサマリー（優先度TOP3）

本レビューは計15件の指摘を含むが、**マージブロック級の問題は以下の3件に集約される**。

| 優先度 | 指摘 | 該当 | 即時推奨アクション |
| --- | --- | --- | --- |
| **P1** | **TOCTOU による上限突破** | `database.go:72-82` | `CountDevices` → INSERT を 1 トランザクション化、または `INSERT ... SELECT WHERE COUNT < limit` で DB 側にアトミック判定を寄せる |
| **P2** | **認証パスの同期化**（goroutine 削除） | `client.go:44-51` | 設定 `device_limit_enforcement=hard/soft` を導入し、soft をデフォルトに。`ErrDeviceLimitReached` 以外は fail-open ＋ メトリクス記録 |
| **P3** | **並行性テストの欠如** | `database_test.go` | 100 goroutine vs limit=10 の race test を追加。Race Detector (`go test -race`) を CI に組み込む |

残り12件は Medium/Low の設計・運用・スタイル負債。P1〜P3 のみ修正されれば最低限のマージ許容ラインに近づく、という判断。
...
内訳: Critical 1 / High 3 / Medium 7 / Low 4（計15件）
...
本課題の時間内で、merge 後の anonymous device 機能に関する後続 PR を `gh` で調査した（**付録 C: 事後検証**）。その結果、本レビューの **#8（DB 種別動作差）/ #11（テスト設計の脆弱性）/ #13（docs 未追記）** は、Grafana 開発の中で実際に後続 PR により是正されたことが確認できた。これは **「事前に review ゲートを通していれば後続で分割対応していたコストを圧縮できた」典型例** であり、本レビューの主張を史実が裏付けている。
...
## 付録C: 事後検証 — Grafana の後続 PR との照合

本レビューの妥当性を検証するため、**PR #79265 がマージされた 2023-12-12 以降の、anonymous device 機能に関する後続 PR・コミット**を `gh search prs` / `gh search commits` で収集した。結果、本レビューの複数指摘が**史実の Grafana 開発過程で実際に問題化していた**ことが確認できた。

### 後続 PR 一覧（#79265 マージ後、関連分）

| PR | タイトル | マージ日 | 本レビューとの対応 |
| --- | --- | --- | --- |
| [#79494](https://github.com/grafana/grafana/pull/79494) | Anonymous: Add device limits to stats | 2023-12-18 | (機能拡張) |
| [#79740](https://github.com/grafana/grafana/pull/79740) | **Anonymous: Add documentation for anonymous devices** | 2023-12-21 | **指摘 #13（docs 未追記）を事後修正** — マージから 9 日後 |
| #79738 / #79747 | anonymousEnabled 関連の UI 修正と backport | 2023-12-20 | (別の表層バグ) |
| [#80028](https://github.com/grafana/grafana/pull/80028) | Anonymous Access: Pagination for devices | 2024-01-04 | (API 拡張) |
| [#80631](https://github.com/grafana/grafana/pull/80631) | **Anonymous: Fix flaky tests** | 2024-01-16 | **指摘 #11（テスト不足）を裏付け** |
| [#81649](https://github.com/grafana/grafana/pull/81649) | **Chore: Fix broken anonymous devices test** | 2024-01-31 | **指摘 #11 を再度裏付け**（後述、特に重要） |
| #88218 | Doc add feature toggle `displayAnonymousStats` | 2024-05-23 | (別機能の docs) |
| [#102991](https://github.com/grafana/grafana/pull/102991) | **spanner: Fix insert or update in anon_device when running on Spanner** | 2025-03-27 | **指摘 #8（DB 種別動作差）を裏付け** |

### 指摘ごとの史実照合

**#13 docs 未追記 — 確定**
PR #79740 のタイトル自体が "Add documentation for anonymous devices"。#79265 の 9 日後に別 PR で事後的に追加されている。本レビューの指摘はこの事実と一致。

**#11 テスト不足（並行性・日付依存） — 複数 PR で裏付け**
- #80631 "Fix flaky tests" はテスト自体の不安定性を修正しており、オリジナル PR のテスト設計が脆弱だったことを示す。
- #81649 "Fix broken anonymous devices test" の PR 本文がきわめて示唆的:
  > "There was a fixed time 2024-01-01. Anonymous devices have an expiration time of 30 days. So today's 2024-01-31 and it is broken :)"

  **固定日付でテストを書いた結果、30日経過で本物のテストが壊れた**。本レビューの #11「テスト設計が核心機能の並行性・時間境界を十分にカバーしていない」という指摘と完全に整合する、実運用での顕在化事例。

**#8 CountDevices のインデックス/DB 動作差 — Spanner 対応で裏付け**
PR #102991 "spanner: Fix insert or update in anon_device when running on Spanner" の本文:
> "Spanner supports INSERT OR UPDATE statement but only when the conflict is on primary key, so we need to use SELECT plus INSERT or UPDATE ourselves."

#79265 が採用した `CreateOrUpdateDevice` の SQL 設計が**DB 横断で動かず、1 年 3 ヶ月後に個別対応**されている。本レビューの「DB 種別動作差（MySQL/Postgres/SQLite 全部でのSQL動作差分）」と同じ構造の問題が、Spanner で実際に発覚した形。

**キャッシュ考慮 — テスト名に痕跡**
#102991 のテスト一覧に `TestIntegrationAnonDeviceService_DeviceLimitWithCache` が存在。オリジナル PR にはなかったテスト名で、**上限判定とローカルキャッシュの相互作用**を後発で検証している。本レビューで言及したキャッシュ観点の懸念と同じ領域。

### 後続 PR で直接の修正が見つからなかった指摘

以下は 2026-04-21 時点の公開 PR 検索範囲では直接の修正 PR が見つからなかった:

- #1 TOCTOU（トランザクション化）
- #2 認証パス同期化（fail-open 化）
- #5 panic recover 削除
- #10 frontend への limit 値露出

これらが**未修正のまま運用されている可能性**と、**本レビューが spec/意図を誤読している可能性**の両方が残る。前者であれば依然としてリスク指摘として有効、後者であれば面談で根拠を問われた際に修正する用意がある。特に #1 TOCTOU は件数規模が小さい環境では顕在化しにくいため、Grafana Cloud のような大規模環境で今後表面化する可能性は残ると推測。

### 検証の結論

- 本レビューの**指摘 #8, #11, #13 の 3 件は、Grafana 開発の後続 PR により事後的に正しさが裏付けられた**。
- 本レビューは「**事前にレビューゲートを通していれば、後続で 3 件の分割対応コストを圧縮できた**」という主張の実証例になっている。
- 残る未検証項目（#1, #2, #5, #10 等）も、指摘の妥当性は低くない。Spec 誤読の可能性は否定できないが、それを検証するには実機 clone と動作確認が必要で、本課題の時間枠を超える。
