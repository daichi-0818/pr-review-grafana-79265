...
重大度: **Critical** = 機能の目的が成立しない / **High** = 可用性・セキュリティに実害 / **Medium** = 設計/運用上の負債 / **Low** = スタイル・ドキュメント
...
| 3 | `client.go:45-49` | **ErrDeviceLimitReached で匿名認証自体を拒否**。公開ダッシュボードが上限超過で閲覧不能化する運用事故リスク。意図が不明確。 | **High** | 自分＋AI両方 |
| 4 | `database.go:85-95` | **`updateDevice` の WHERE 句のバグ**: `BETWEEN device.UpdatedAt-30d AND device.UpdatedAt+1min` は「DB 既存行の updated_at」ではなく「これから書き込む値」基準。`rowsAffected=0` を一律 `ErrDeviceLimitReached` にしているため、単に device_id 未登録のケースと区別できない。 | **High** | AI（自分は「時間窓が怪しい」まで）|
...
| 10 | `frontend_settings.go:195` / `config.ts:197` | **limit 値を未認証フロントに公開 = 情報漏洩**。攻撃者は `limit+1` 個のデバイスIDで正当ユーザーを締め出す DoS コストを正確に計算できる。匿名公開ダッシュボード環境こそ影響が大きい。 | **Medium** | 自分＋AI両方 |
...
| 12 | PR全体 | **PR本文チェックリスト未消化**: 本文内に `[ ] It works as expected`, `[ ] behind a feature toggle`, `[ ] docs are updated` の 3 項目とも未チェック。feature toggle なしでマージされており Grafana のリリースポリシー違反。 | **Medium** | 自分（AIは docs 未追記のみ言及） |
...
| 16 | PRガバナンス | **作者自身が `REVIEW_REQUIRED` 状態のまま self-merge**。Grafana のレビュープロセス上の逸脱。 | **Low**（コードではなくプロセス） | 自分＋AI両方 |
...
### 5.3 #4 `updateDevice` の WHERE 句を意味論に合わせて書き直す

```go
// 意図: 既存 device_id かつ直近30日以内に観測のある行を更新
const q = `UPDATE anon_device
           SET client_ip = ?, user_agent = ?, updated_at = ?
           WHERE device_id = ? AND updated_at >= ?`
args := []interface{}{
    device.ClientIP, device.UserAgent, now,
    device.DeviceID, now.Add(-anonymousDeviceExpiration),
}
// rowsAffected == 0 は「既存行なし」or「古すぎて除外」
// → ErrDeviceLimitReached ではなく ErrDeviceNotTrackable 等に改名し、
//   呼び出し側で「上限超過 or 未登録」を切り分ける
```
...
- **#10 情報漏洩** は frontend settings DTO に設定値を足している差分を見て「匿名にも配るのか」で違和感を持った。
...
2. **High 4件** が認証パスの可用性・設計・運用に及ぼす影響が大きい。特に #2 の同期化は大規模公開ダッシュボードで SLA を損なう典型パターン。#3 は新規匿名ユーザーの閲覧拒否が設計意図なのか副作用なのかが PR 本文から読み取れない。
3. テストが**新機能の核心である並行性を全くカバーしていない**。
4. PR 本文のチェックリスト（feature toggle / docs / 動作確認）がすべて未チェックのまま、かつ作者自身が `REVIEW_REQUIRED` 状態のまま merge しており、プロセス面でも問題がある。
